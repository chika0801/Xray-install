...
2. 参考[sing-box Windows 客户端使用方法](https://github.com/chika0801/sing-box-examples/tree/main/Tun)，使用这个[sing-box 客户端配置文件](https://github.com/chika0801/Xray-install/blob/main/Tun/sing-box_client_config.json)。简易说明：由 **sing-box** 提供 **Tun** 模式（透明代理环境），接管程序发出的网络访问请求（域名或IP）。域名进入 **"dns"** 部分，按预设的规则进行DNS解析，解析返回的IP（直接请求的IP）进入 **"route"** 部分。使用 **"sniff"** 参数，获得IP被解析前的域名（直接请求的IP无域名信息）。IP和域名作为条件，按预设的规则进行分流。并且不使用 **"sniff_override_destination"** 参数。即sing-box发送Xray的请求是IP，不把IP还原成域名发送到Xray。
