...
2.在你的手机中打开文件管理APP，进入后找到你刚才复制进来的apk文件，并安装它。

3.进入v2rayNG，点击左上角的“≡”打开菜单，点击“设置”，勾选“启用本地DNS”，“域名策略”建议改为“AsIs”，“预定义规则”改为“绕过局域网及大陆地址”。
...
<strike>简单解释一下工作流程及大致原理：手机APP发出连接请求，请求解析域名，请求通过v2rayNG，进入v2rayNG内部的Xray-core处理。请求先进入DNS模块，若域名包含在数据库GeoSite的cn分类里，用DNS223.5.5.5进行解析，若返回的IP包含在数据库GeoIP里，则返回IP结果给手机APP，若为否，用DNS1.1.1.1进行解析，返回IP结果给手机APP。手机APP拿到IP，发起请求，请求进入routing模块，若域名包含在数据库GeoSite的cn分类，则本地直连。</strike>
